...
export default function BrandAdminBenefits() {
  const [search, setSearch] = useState('');
  const { brandInfo } = useBrandAdmin();
  const queryClient = useQueryClient();

  const { data: benefits = [], isLoading } = useQuery({
    queryKey: ['brand-admin', 'benefits', brandInfo?.brand_id],
    queryFn: () => listBenefitsAdmin(brandInfo!.brand_id),
  ...
  return (
    <BrandAdminLayout title="Beneficios" description="Gestionar beneficios para usuarios verificados">
                ...
                <tr key={benefit.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Link to={`/marca/beneficios/${benefit.id}`}>
